...
**Empty ASO blocks (language header with no title/subtitle/keywords):** The locale is still prepared. App name/subtitle use App Info for that locale when present; otherwise primary App Info. For locales that are not in the latest released App Store version, `keywords`, What's New, and Support/Marketing URLs are taken from primary (or config fallback for What's New). Locales that already exist in the released version keep their stored values for empty fields (primary is not applied field-by-field). Description is never copied from primary (sheet or same locale in release only).

**Locales missing from ASO (e.g. screenshot-only):** Screenshot locales are included in metadata preparation with the same fallback rules as empty ASO blocks.
