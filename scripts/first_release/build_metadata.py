...
def build(listing: dict, out_dir: Path) -> dict:
    ...
    # Подписки: создания продуктов с нуля в прогоне пока нет. Обычные прогоны
    # умеют дописывать локализации к продуктам, которые в ASC уже есть, а
    # завести группу, тарифы и цены — отдельная работа. Молча пропустить блок
    # нельзя: прогон закончился бы успехом, а приложение осталось без пейволла.
    if dig(listing, "subscriptions") is not None:
        problems.append(
            "subscriptions: в файле есть блок подписок, а создавать продукты с нуля "
            "прогон ещё не умеет. Завести группу и тарифы в App Store Connect руками, "
            "затем убрать блок из файла — локализации допишет обычный прогон")
