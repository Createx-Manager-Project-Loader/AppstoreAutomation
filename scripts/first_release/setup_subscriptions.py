    ...
    def product(self, group_id: str, product: dict, notes: str | None) -> str:
        ...
        try:
            ...
            if "already been used" in str(error):
                # Живой продукт с таким id мы бы нашли выше и обновили. Раз не
                # нашли, а Apple говорит «занят» — продукт был удалён. Через
                # API он не виден нигде (коллекцию подписок ASC не отдаёт
                # вовсе), а идентификатор остаётся занятым навсегда. Проверено.
                raise Problem(
                    f"product id «{product_id}» занят удалённым продуктом. Живой продукт "
                    "прогон нашёл бы и обновил, но удалённый через API не виден, а id "
                    "Apple обратно не отдаёт. Нужен другой id в файле листинга. "
                    "На будущее: продукты подписок не удаляют, а снимают с продажи") from error
            raise
        return created["data"]["id"]
